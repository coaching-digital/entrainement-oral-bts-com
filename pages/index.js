...
const makeSystemP1 = (cibl\u00e9=null, s\u00e9v\u00e8re=false, isSimulation=false) => {
  ...
  const filteredP1 = cibl\u00e9?.key ? Object.entries(QUESTIONS_P1).filter(([k,v]) => k === cibl\u00e9.key) : [];
  const bank = filteredP1.length > 0 ? filteredP1 : Object.entries(QUESTIONS_P1);
...
const makeSystemP2 = (cibl\u00e9=null, s\u00e9v\u00e8re=false, isSimulation=false) => {
  const niveau = s\u00e9v\u00e8re ? "exigeantes" : "classiques";
  const filteredP2 = cibl\u00e9?.key ? Object.entries(QUESTIONS_P2).filter(([k,v]) => k === cibl\u00e9.key) : [];
  const bank = filteredP2.length > 0 ? filteredP2 : Object.entries(QUESTIONS_P2);
  const bankStr = bank.map(([k,v]) => "=== " + k + " : " + v.label + " ===\n" + v[niveau].map((q,i) => (i+1)+". "+q).join("\n")).join("\n\n");
              ...
              <>
                ...
                <div style={{ background: C.bg2, borderRadius: 16, padding: "12px" }}>
                  ...
                  <div className="criteria-grid" style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: "1rem" }}>
                    {FICHES.p1.map((f) => (
                      <button key={f.id} onClick={() => startPhase(1, "ciblé", { part: "p1", label: f.label, key: f.id })} style={{ padding: "10px 12px", borderRadius: 12, border: `1px solid ${f.bg}`, background: f.bg, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div className="criteria-grid" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {FICHES.p2.map((f) => (
                      <button key={f.id} onClick={() => startPhase(2, "ciblé", { part: "p2", label: f.label, key: f.id })} style={{ padding: "10px 12px", borderRadius: 12, border: `1px solid ${f.bg}`, background: f.bg, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 10 }}>
                        <span style={{ fontSize: 11, fontWeight: 700, color: f.color, minWidth: 22, flexShrink: 0 }}>{f.id}</span>
                        <span style={{ fontSize: 13, color: C.text, flex: 1 }}>{f.label}</span>
                        <span style={{ fontSize: 11, color: f.color, flexShrink: 0 }}>→</span>
                      </button>
